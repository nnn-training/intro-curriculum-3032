  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const deleteOneTimeToken = params.get('oneTimeToken');
        Post.findByPk(id).then((post) => {//idで投稿内容を取得している取得したそのIDの投稿内容すべて（ID以外の情報contentなども）をthenに渡している
          if(!(deleteOneTimeToken && id)){
            return util.handleBadRequest(req, res);//ここは削除フォームで必要なidとワンタイムトークンがあるか確認している
          }
          if(!(deleteOneTimeToken === oneTimeTokenMap.get(req.user))){
